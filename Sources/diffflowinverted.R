...
networkData <- networkData %>% group_by(jname) %>% mutate(sum = sum(fij))
...
flowtest <- filter(flowtest, country=='Brazil')

flowtest <- flowtest %>% group_by(jfromname3, ifromname3) %>% mutate(count = n())
...
networkDatatest <- networkDatatest %>% group_by(jname) %>% mutate(sum = sum(fij))
...
ggplot(flowcomp, aes(iname, jname, fill= fij)) + 
  geom_tile() + xlab("Source") + ylab("Destination") + labs(fill = "% of difference") +
